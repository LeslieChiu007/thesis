# Streams_Nonhomo_Warmup_run1_0815.py
...
def patient(env, name, ps, num_beds, proportion, stream_id, all_data):
    ...
    # Only count arrivals after warm-up period
    if arrive > all_data["warm_up_period"]:
        all_data["total_arrivals"][stream_id] += 1  # Increment the count of arrivals
        all_data["arrival_rates"][stream_id].append((env.now, all_data["total_arrivals"][stream_id]/(env.now - all_data["warm_up_period"])))
    ...
    with ps.bed.request() as request:
        ...
        if arrive > all_data["warm_up_period"]:
            all_data["wait_times"][stream_id].append(wait)
            if wait > 0:
                all_data["nonzero_wait_times"][stream_id].append(wait)
        ...
        if arrive > all_data["warm_up_period"]:
            ...
            all_data["sojourn_times"][stream_id].append(wait + service_time)
...
def run_simulation(params, warm_up_period, sim_time):
    all_data = {key: {i: [] for i in range(1, 9)} for key in [
        'arrival_rates', 'leaving_patients', 'queue_lengths', 'num_patients', 'wait_times',
        'nonzero_wait_times', 'service_times', 'sojourn_times', 'utilization', 'throughput']}
    all_data['warm_up_period'] = warm_up_period
    all_data['total_arrivals'] = {i: 0 for i in range(1, 9)}  # New key to track arrivals
    all_data['total_service_time'] = {i: 0 for i in range(1, 9)}    # to calculate utilization
    all_data['served'] = {i: 0 for i in range(1, 9)}                # to calculate throughput
...
# def calculate_arrival_rate(all_data, sim_time):
#     arrival_rates = {}
#     effective_simulation_time = sim_time - all_data['warm_up_period']  # Time after warm-up
#
#     for stream_id, total_arrivals in all_data['total_arrivals'].items():
#         arrival_rate = total_arrivals / effective_simulation_time
#         arrival_rates[stream_id] = arrival_rate
#         print(f"Simulated Arrival Rate for Stream {stream_id} (post warm-up): {arrival_rate:.4f}")
#
            ...
            if isinstance(data, list) and data:
                try:
                    ...
                    if average_line is not None:
                        plt.axhline(y=average_line[stream_id-1], color='r', linestyle='--', label=f'Average {ylabel} (Theoretical)= {average_line[stream_id-1]:.2f}')
                    plt.xlabel(xlabel)
                    plt.ylabel(ylabel)
        ...
        if stream_id in stream_ids:
            if isinstance(data, list) and data:
                try:
                    ...
                    max_value = max(data)
                    print(f'Mean {ylabel} for Stream {stream_id} (Simulation): {mean_value}')
                    print(f'Max {ylabel} for Stream {stream_id} (Simulation): {max_value}')
                    plt.subplot(2, 2, (stream_id - 1) % 4 + 1)
...
def calculate_average_metrics(all_data, params):     # from queueing theory, dashlines
    average_L = []
    average_Lq = []
    for param in params:
        ...
        # Calculate L and Lq
        L = (arrival_rate / service_rate) + (Pw * rho / (1 - rho))  # formula 4.11
        Lq = L - (arrival_rate / service_rate)                      # formula 4.12

        print(f"In theory, Stream {param['stream_id']}: L = {L}, Lq = {Lq}")

        average_L.append(L)
        average_Lq.append(Lq)
    return average_L, average_Lq

def visualize_results(all_data, params, output_dir='nonhome_warmup_streams_plots_1run'):
    ...
    plot_distribution(all_data['wait_times'], 'Wait Time Distribution', 'Wait Time', 'Wait Time', colors, range(1, 5), os.path.join(output_dir, 'wait_times_1_4.png'))
    plot_distribution(all_data['wait_times'], 'Wait Time Distribution', 'Wait Time', 'Wait Time', colors, range(5, 9), os.path.join(output_dir, 'wait_times_5_8.png'))
    plot_distribution(all_data['nonzero_wait_times'], 'Nonzero Wait Time Distribution', 'Nonzero Wait Time', 'Nonzero Wait Time', colors, range(1, 5), os.path.join(output_dir, 'nonzero_wait_times_1_4.png'))
    plot_distribution(all_data['nonzero_wait_times'], 'Nonzero Wait Time Distribution', 'Nonzero Wait Time', 'Nonzero Wait Time', colors, range(5, 9), os.path.join(output_dir, 'nonzero_wait_times_5_8.png'))
    ...
    plot_distribution(all_data['sojourn_times'], 'Sojourn Time Distribution', 'Sojourn Time', 'Sojourn Time', colors, range(1, 5), os.path.join(output_dir, 'sojourn_times_1_4.png'))
    plot_distribution(all_data['sojourn_times'], 'Sojourn Time Distribution', 'Sojourn Time', 'Sojourn Time', colors, range(5, 9), os.path.join(output_dir, 'sojourn_times_5_8.png'))
    ...
    plot_metric_over_time(all_data['arrival_rates'], 'Arrival Rate Over Time', 'Time', 'Arrival Rate', colors, range(1, 5), os.path.join(output_dir, 'arrival_rates_1_4.png'))
    plot_metric_over_time(all_data['arrival_rates'], 'Arrival Rate Over Time', 'Time', 'Arrival Rate', colors, range(5, 9), os.path.join(output_dir, 'arrival_rates_5_8.png'))
...
def main():
    ...
    # calculate_arrival_rate(all_data, sim_time)
